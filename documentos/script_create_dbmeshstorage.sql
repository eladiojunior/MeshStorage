...
CREATE TABLE tb_server_storage_metrics (
  ...
  ID_SERVER_STORAGE BIGINT NOT NULL,
  ...
  QT_REQUEST_LAST_MINUTE_STORAGE INTEGER UNSIGNED NOT NULL,
  QT_ERRORS_LAST_REQUEST_STORAGE INTEGER UNSIGNED NOT NULL,
  DH_LAST_AVAILABLE_STORAGE DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
  PRIMARY KEY(ID_SERVER_STORAGE_METRICS),
  INDEX IX_ID_SERVER_STORAGE(ID_SERVER_STORAGE),
  FOREIGN KEY(ID_SERVER_STORAGE)
    REFERENCES tb_server_storage(ID_SERVER_STORAGE)
      ON DELETE NO ACTION
      ON UPDATE NO ACTION
);
...
CREATE TABLE tb_file_storage (
  ...
  INDEX TB_FILE_STORAGE_FKIndex1(ID_APPLICATION),
